      ...
      <swiper-item v-for="(i,index) in items" :key="index">
        <view class="showImages">
          ...
          <view class="showImages_main">
            ...
            <view class="showfour">
              <view class="showfour_con space-between" v-if="current == 1">
                <view v-for="(item,index) in fourDraw" :key="index">
                  <view class="showImages_main_con">
                    <img :src="item.pCover" alt class="showImages_main_con_i" />
                    <img
                      src="../../../static/choose.png"
                      alt
                      class="showImages_main_con_e"
                      @click="chooseFour(index)"
                    />
                  </view>
                  <view>
                    <view class="showImages_main_ct">{{item.pName}}</view>
                    <view class="showImages_main_p">
                      <text>￥</text>
                      {{item.pPricef}}
                    </view>
...
export default {
  ...
  onShow() {
    ...
    this.singleDraw =
      global.manageCurrent == 0
        ? global.manageChooseGoods
        : global.manageDetails.goodsList[0];
    if (global.manageFourIndex) {
      // this.fourDraw[global.manageFourIndex] = global.manageChooseGoods
      this.$set(
        this.fourDraw,
        global.manageFourIndex,
        global.manageChooseGoods
      );
    } else {
      this.fourDraw = global.manageDetails.goodsList;
      this.fourDraw = this.fourDraw.slice(0, 4);
    }
    this.GetWXAppletsCode();
  },
  onReady() {
    // this.draw()
  },
  methods: {
    async GetWXAppletsCode() {
      let res = await this.$api.GetWXAppletsCode({
        muId: this.muId,
      });
    ...
    async draw() {
      ...
      if (this.current == 0) {
        ...
        var images = [
          this.fourDraw[0].pCover,
          this.fourDraw[1].pCover,
          this.fourDraw[2].pCover,
          this.fourDraw[3].pCover,
          this.user.uPic,
          this.userInfo.avatarUrl,
          this.ShopImageCode,
        ];
    ...
    drawFourImage(images) {
      wx.showLoading({
        title: "生成中",
      });
      var context = uni.createCanvasContext("firstCanvas");
      context.drawImage("../../../static/canvas.png", 0, 0, 375, 663);
      context.drawImage(images[6].path, 242, 490, 78, 78);

      // context.drawImage(images[images.length-1].path, 243, 480, 75, 75 )

      // this.fillText(context,9,'#999999',"长按识别 即可购买", 243, 570, 300);
      ...
      //第一组图
      this.drawImage(context, 127, 112, 56, 124, 83, images[0].path);
      // context.drawImage('../../../static/choose.png', 140, 130, 35, 35);
      this.fillText(
        context,
        13,
        "#333",
        this.namelength(this.fourDraw[0].pName),
        60,
        255,
        95
      );
      this.fillText(context, 12, "#F61E45", "￥", 60, 278);
      this.fillText(context, 19, "#F61E45", this.fourDraw[0].pPricef, 72, 278);
      //第二组图
      this.drawImage(context, 127, 112, 193, 124, 83, images[1].path);
      // context.drawImage('../../../static/choose.png', 278, 130, 35, 35);
      this.fillText(
        context,
        13,
        "#333",
        this.namelength(this.fourDraw[1].pName),
        197,
        255,
        95
      );
      this.fillText(context, 12, "#F61E45", "￥", 197, 278);
      this.fillText(context, 19, "#F61E45", this.fourDraw[1].pPricef, 207, 278);
      // this.fillText(context,19,'#F61E45',this.fourDraw[1].pPricef, 207, 278)
      //第三组图
      this.drawImage(context, 127, 112, 56, 290, 83, images[2].path);
      // context.drawImage('../../../static/choose.png', 140, 130, 35, 35);
      this.fillText(
        context,
        13,
        "#333",
        this.namelength(this.fourDraw[2].pName),
        60,
        419,
        95
      );
      this.fillText(context, 12, "#F61E45", "￥", 60, 440);
      this.fillText(context, 19, "#F61E45", this.fourDraw[2].pPricef, 72, 440);
      //第四组图
      this.drawImage(context, 127, 112, 193, 290, 83, images[3].path);
      // context.drawImage('../../../static/choose.png', 278, 130, 35, 35);
      this.fillText(
        context,
        13,
        "#333",
        this.namelength(this.fourDraw[3].pName),
        197,
        419,
        95
      );
      this.fillText(context, 12, "#F61E45", "￥", 197, 440);
      this.fillText(context, 19, "#F61E45", this.fourDraw[3].pPricef, 207, 440);
      context.drawImage("../../../static/dashed.png", 24, 470, 327, 1);
      this.fillText(context, 12, "#333", this.userInfo.nickName, 100, 530);
      this.fillText(context, 10, "#999", "我通过驿货分享了一件商品", 100, 550);
      this.fillText(context, 16, "#fff", "驿货商城", 157, 640);
      this.fillText(context, 9, "#999", "长按识别 即可购买", 242, 580);
      this.drawImage(context, 52, 52, 56, 57, 35, images[4].path);
      this.drawImage(context, 38, 38, 56, 515, 19, images[5].path);
      context.draw();
      var that = this;
      setTimeout(function () {
        wx.hideLoading();
